...
class TestPsycopg2TypeConversion(TestCase):
    ...
    def test_conv_timetz(self):
        ...
        self.assertEquals(c(''), None)
        self.assertEquals(
            c('12:44:01+01:00'), time(12, 44, 1, 0, TZStub(1, 0))
        )
        self.assertEquals(
            c('12:44:01.037-00:30'), time(12, 44, 1, 37000, TZStub(0, -30))
        )
    ...
    def test_conv_timestamp(self):
        ...
        self.assertEquals(c(''), None)
        self.assertEquals(
            c('2001-03-02 12:44:01'), datetime(2001, 3, 2, 12, 44, 1)
        )
        self.assertEquals(
            c('2001-03-02 12:44:01.037'),
            datetime(2001, 3, 2, 12, 44, 1, 37000),
        )
        self.assertEquals(
            c('2001-03-02 12:44:01.000001'), datetime(2001, 3, 2, 12, 44, 1, 1)
        )
    ...
    def test_conv_timestamptz(self):
        ...
        self.assertEquals(
            c('2001-03-02 12:44:01+01:00'),
            datetime(2001, 3, 2, 12, 44, 1, 0, TZStub(1, 0)),
        )
        self.assertEquals(
            c('2001-03-02 12:44:01.037-00:30'),
            datetime(2001, 3, 2, 12, 44, 1, 37000, TZStub(0, -30)),
        )
        self.assertEquals(
            c('2001-03-02 12:44:01.000001+12:00'),
            datetime(2001, 3, 2, 12, 44, 1, 1, TZStub(12, 0)),
        )
        self.assertEquals(
            c('2001-06-25 12:14:00-07'),
    ...
    def test_conv_interval(self):
        ...
        self.assertEquals(
            c('2 days 03:20:15.123456'),
            timedelta(
                days=2, hours=3, minutes=20, seconds=15, microseconds=123456
            ),
        )
    ...
    def test_registerTypes(self):
        ...
        for typename in (
            ...
            if not result:
                ...
                self.fail(
                    "did not register %s (%d): got None, not Z%s"
...
class TestISODateTime(TestCase):
    ...
    basic_dates = (('20020304', (2002, 3, 4)), ('20000229', (2000, 2, 29)))
    ...
    def setUp(self):
        from psycopg2da.adapter import (
            parse_date,
            parse_datetime,
            parse_datetimetz,
            parse_time,
        ...
        for ds, d in self.basic_dates:
            for ts, t in self.basic_times:
                for tz, off in self.basic_tzs:
                    for sep in self.time_separators:
                        self.assertEqual(
                            self.parse_datetimetz(ds + sep + ts + tz),
                            d + t + (off,),
                        )
